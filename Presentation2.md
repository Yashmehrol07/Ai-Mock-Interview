...
### Technologies Used
- **Node.js & Express.js**: ⭐ **USE (Mandatory)** - The fundamental server-side framework to handle high-performance API routing and fast logic execution.
- **MongoDB & Mongoose**: ⭐ **USE (Mandatory)** - A robust NoSQL database paired with an Object Data Modeling (ODM) library for storing complex relations like sessions, generated questions, and user profiles effortlessly.
- **JSON Web Tokens (JWT) & bcryptjs**: ⭐ **USE (Mandatory)** - Critical security implementations managing token-based authentication and secure password salting/hashing.
- **Google Gemini API**: ⭐ **USE (Mandatory)** - The powerful AI engine (`@google/genai`) tasked with dynamically generating customized interview questions.
- **dotenv & cors**: **USE (Mandatory)** - Ensuring environment variable security and managing Cross-Origin Resource Sharing from our Vite application.
...
### Technologies Used
- **React 19 & Vite**: ⭐ **USE (Mandatory)** - Next-generation component-based UI rendering, bundled with Vite for incredibly fast Hot Module Replacement (HMR) during development.
- **Tailwind CSS v4 & Framer Motion**: ⭐ **USE (Mandatory)** - Utility-first layout styling combined with highly polished micro-interactions, animations, and transitions to ensure a modern feel.
- **React Router DOM**: ⭐ **USE (Mandatory)** - Fast and efficient client-side declarative routing between pages (Login, Dashboard, Prep) without requesting new HTML documents remotely.
- **Axios**: ⭐ **USE (Mandatory)** - A robust promise-based HTTP client customized to inject authentication headers locally and communicate securely with the Express API.
- **React Markdown & Remark GFM**: ⭐ **USE (Mandatory)** - Critical for rendering rich code syntax and standard markdown formatting dynamically imported from Google Gemini responses.
